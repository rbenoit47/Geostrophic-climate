...
#
use File::Basename;
# use a modified input file because doing ALL tables
open (PARAMS, "<prep_all_tables.txt");
while (<PARAMS>) {
   chop;
   @line = split;
   $dirname = @line[0];
...
close (INFOS);
#
my @files = glob("$dirname/*/*$extnamein");
#
# main loop on whole directory
foreach my $fname (@files) {
	($dir,$fn,$ext) = fileparse ($fname,$extnamein); #".ef");
	$tname = join $dir,$fn,$extnameout;   # construire nom des table txt -> ef
	print "generate ",$tname,"\n";
	open (CLASSES, "<$fname");
	#
	$tline = 0;
	$readheader = 1;
